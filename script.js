const experation = document.querySelector('[data-experation-year]');
...
document.addEventListener('keydown', e => {
    ...
    switch (key) {
        ...
        case 'Backspace': {
            if (input.selectionStart === 0 && input.selectionEnd === 0) {
                const prev = input.previousElementSibling;
                if (prev === null) return e.preventDefault();
...
const isConnectedInput = (input) => {
    ...
    return parent !== null && input.matches('input');
}